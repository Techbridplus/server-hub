...
export function ChatInterface({ group, currentUserId, isAdmin, defaultChannelId }: ChatInterfaceProps) {
  ...
  return (
    <div className="flex h-full w-full flex-col md:flex-row">
      ...
      {/* Desktop sidebar */}
      <div className="hidden w-60 flex-col border-r md:flex">
        <div className="p-4">
          <h2 className="text-lg font-semibold">{group.name}</h2>
          <p className="text-sm text-muted-foreground">{group.description}</p>
        </div>
      ...
      <div className="flex flex-1 flex-col">
        <div className="flex h-14 items-center justify-between border-b px-4">
          <div className="flex items-center gap-x-2">
            {activeChannel && group.channels.find((c: any) => c.id === activeChannel)?.type === "text" ? (
              <Hash className="h-5 w-5" />
            ) : (
              <Volume2 className="h-5 w-5" />
            )}
            <span className="font-medium">
              {activeChannel ? group.channels.find((c: any) => c.id === activeChannel)?.name : "Select a channel"}
            </span>
          </div>
          <Button
            variant="ghost"
